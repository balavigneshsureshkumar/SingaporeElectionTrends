...
# 📊 Advanced Singapore Election Analysis (1959-2025)
...
## 1a. Sparkline Plot by Region

```{r}
# Prepare data for regional spark plots
...
## 1b. Individual Region Sparkline Plots

### Central Region: Party Vote Share Trends

```{r}
central_data <- region_party_performance %>% filter(region == "Central")
ggplot(central_data, aes(x = year, y = vote_share_region, color = party_clean, group = party_clean)) +
  geom_xspline(size = 1, spline_shape = -0.6) +
  geom_point(size = 2) +
  geom_text(aes(label = round(vote_share_region, 2)), vjust = -1.5, size = 3, show.legend = FALSE) +
  facet_wrap(~ party_clean, scales = "free_y", ncol = 1, strip.position = "left") +
  scale_x_continuous(breaks = unique(central_data$year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
  ...
  labs(
    title = "Sparkline Trend: Vote Share by Party in Central",
    subtitle = "Smoothed curves with actual vote share labels per year"
  )
```

### North Region: Party Vote Share Trends

```{r}
north_data <- region_party_performance %>% filter(region == "North")
ggplot(north_data, aes(x = year, y = vote_share_region, color = party_clean, group = party_clean)) +
  geom_xspline(size = 1, spline_shape = -0.6) +
  geom_point(size = 2) +
  geom_text(aes(label = round(vote_share_region, 2)), vjust = -1.5, size = 3, show.legend = FALSE) +
  facet_wrap(~ party_clean, scales = "free_y", ncol = 1, strip.position = "left") +
  scale_x_continuous(breaks = unique(north_data$year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
  ...
  labs(
    title = "Sparkline Trend: Vote Share by Party in North",
    subtitle = "Smoothed curves with actual vote share labels per year"
  )
```

### West Region: Party Vote Share Trends

```{r}
west_data <- region_party_performance %>% filter(region == "West")
ggplot(west_data, aes(x = year, y = vote_share_region, color = party_clean, group = party_clean)) +
  geom_xspline(size = 1, spline_shape = -0.6) +
  geom_point(size = 2) +
  geom_text(aes(label = round(vote_share_region, 2)), vjust = -1.5, size = 3, show.legend = FALSE) +
  facet_wrap(~ party_clean, scales = "free_y", ncol = 1, strip.position = "left") +
  scale_x_continuous(breaks = unique(west_data$year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
  ...
  labs(
    title = "Sparkline Trend: Vote Share by Party in West",
    subtitle = "Smoothed curves with actual vote share labels per year"
  )
```

### East Region: Party Vote Share Trends

```{r}
east_data <- region_party_performance %>% filter(region == "East")
ggplot(east_data, aes(x = year, y = vote_share_region, color = party_clean, group = party_clean)) +
  geom_xspline(size = 1, spline_shape = -0.6) +
  geom_point(size = 2) +
  geom_text(aes(label = round(vote_share_region, 2)), vjust = -1.5, size = 3, show.legend = FALSE) +
  facet_wrap(~ party_clean, scales = "free_y", ncol = 1, strip.position = "left") +
  scale_x_continuous(breaks = unique(east_data$year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
  ...
  labs(
    title = "Sparkline Trend: Vote Share by Party in East",
    subtitle = "Smoothed curves with actual vote share labels per year"
  )
```

### North-East Region: Party Vote Share Trends

```{r}
ne_data <- region_party_performance %>% filter(region == "North-East")
ggplot(ne_data, aes(x = year, y = vote_share_region, color = party_clean, group = party_clean)) +
  geom_xspline(size = 1, spline_shape = -0.6) +
  geom_point(size = 2) +
  geom_text(aes(label = round(vote_share_region, 2)), vjust = -1.5, size = 3, show.legend = FALSE) +
  facet_wrap(~ party_clean, scales = "free_y", ncol = 1, strip.position = "left") +
  scale_x_continuous(breaks = unique(ne_data$year)) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  theme_minimal(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold"),
  ...
  labs(
    title = "Sparkline Trend: Vote Share by Party in North-East",
    subtitle = "Smoothed curves with actual vote share labels per year"
  )
```
...
::: heatmap-container
#### 🔥 Party Support Heatmap by Region (2025)
